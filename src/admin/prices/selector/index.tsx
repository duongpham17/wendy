...
const Create = ({setCreate, create}:Props) => {
    ...
    const initialState = { type: "", prices: [], createdAt: Date.now()};

    const {values, onChange, onSubmit, edited, loading, onClear} = useForm(initialState, callback);

    async function callback(){
        try{
            const res = await api.post("/prices", values);
    ...
    return (
        <Cover onClose={() => setCreate(false)} open={create}>
            <form onSubmit={onSubmit}>
                <Input label1={"Price Category"} value={values?.type} name="type" onChange={onChange}/>
